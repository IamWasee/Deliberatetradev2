...
import { PROCESS_LABELS, computeProcess, emotionExpectancy, equityCurve, rollingR, setupStats } from "../lib/coaching";
    ...
    <div className="h-full overflow-y-auto p-3 md:p-4">
      <div className="max-w-[1180px] mx-auto space-y-3.5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
          ...
          <Stat label="Avg R / trade" v={s.trades.length ? fmtR(avgR) : "—"} sub={`${s.trades.length} closed trades`} tone={avgR >= 0 ? "up" : s.trades.length ? "down" : undefined} />
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-3.5">
          ...
          <div className="space-y-3.5">
            ...
            <div className="panel p-4">
              ...
              {roll.length === 0 ? <Empty title="No sample yet" body="Your rolling expectancy appears after the first closed trade." /> : (
                  ...
                  {roll.slice(-40).map((r, i) => (
                    <div key={i} className="flex-1 flex flex-col justify-end items-stretch group relative">
                      <div className="rounded-sm transition-all duration-300"
                        style={{
                          height: `${Math.min(100, Math.max(6, Math.abs(r) * 46))}%`,
                          background: r >= 0 ? "rgba(47,185,140,0.75)" : "rgba(224,86,79,0.75)",
                      ...
                      <span className="absolute -top-6 left-1/2 -translate-x-1/2 num text-[9.5px] px-1 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10"
                        style={{ background: "#0a1120", border: "1px solid #1c2942", color: r >= 0 ? "#2fb98c" : "#e0564f" }}>
                        {fmtR(r)}
                      </span>
                    </div>
                  ))}
          ...
          {/* right rail: process + emotion + violations */}
          <div className="space-y-3.5">
            <div className="panel p-4 flex flex-col items-center">
              <Gauge value={proc.score} label="Process score" size={132} />
              <p className="text-[11.5px] text-fog-500 text-center mt-2 leading-snug max-w-[260px]">
                Weighted: adherence 28 · journal 22 · risk 20 · emotion 16 · setup 14. This number outranks P&L everywhere on this platform.
              </p>
            ...
            <div className="panel p-4">
              ...
              {s.violations.slice(0, 6).map((v) => (
                <div key={v.id} className="py-2 border-b border-line-soft last:border-0">
                  <p className="text-[12px] font-semibold" style={{ color: "#e8837d" }}>{v.rule}</p>
