...
export default function Terminal() {
  ...
  const ind = useMemo(
    () => (m ? computeIndicators(m.candles, s.indicators) : { overlays: [], panes: [], showVolume: false }),
  ...
  return (
    <div className="h-full flex flex-col gap-3 p-3 overflow-hidden">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-[218px_1fr_288px] gap-3 min-h-0 overflow-y-auto">
        {/* watchlist + news */}
        <div className="flex flex-col gap-3 min-h-0" data-tour="watchlist">
        ...
        {/* chart column */}
        <div className="flex flex-col gap-3 min-h-0" data-tour="chart">
          <div className="panel p-3.5">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mb-2">
              ...
              <button onClick={() => setIndOpen(true)}
                className="btn btn-ghost !py-1.5 !px-3 !text-[11.5px] flex items-center gap-1.5">
                <Ic.flask size={13} /> Indicators
                <span className="num text-[9.5px] px-1.5 py-0.5 rounded-full" style={{ background: "#111b30", border: "1px solid #1c2942" }}>{s.indicators.length}</span>
              </button>
        ...
        <div className="min-h-0" data-tour="ticket">
          {pos ? <ManagePanel key={pos.id} posId={pos.id} /> : (
            <EntryTicket gateOk={gate.ok} gateReason={gate.reason}
              onTradeIntent={() => setDisclaimerPending(true)} />
          )}
        </div>
      </div>
    ...
    <div className="h-full flex flex-col gap-3 p-3 overflow-hidden animate-fade-in" aria-busy="true" aria-label="Loading terminal">
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-[218px_1fr_288px] gap-3 min-h-0">
        <div className="panel p-2.5 space-y-1">
          ...
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="flex items-center gap-2 py-1.5">
              <Skeleton w={34} h={11} /><Skeleton w={44} h={16} /><Skeleton h={11} className="ml-auto" w={52} />
            </div>
          ))}
        </div>
        <div className="flex flex-col gap-3 min-h-0">
          <div className="panel p-3.5">
            <div className="flex items-center gap-3 mb-3">
              <Skeleton w={70} h={18} /><Skeleton w={110} h={24} className="ml-auto" /><Skeleton w={90} h={16} r={99} />
            </div>
            <Skeleton h={288} r={10} />
          </div>
        ...
        <div className="panel p-4 space-y-3">
          <Skeleton w={90} h={9} /><Skeleton h={38} r={9} /><Skeleton h={34} r={9} /><Skeleton h={34} r={9} />
          <div className="grid grid-cols-2 gap-2"><Skeleton h={34} r={9} /><Skeleton h={34} r={9} /></div>
          <Skeleton h={92} r={10} /><Skeleton h={42} r={9} />
        </div>
      </div>
    </div>
  );
...
function EntryTicket({ gateOk, gateReason, onTradeIntent }: { gateOk: boolean; gateReason: string; onTradeIntent: () => void }) {
  ...
  useEffect(() => {
    ...
    const st = px - dir * 1.5 * a;
    setStop(Number(st.toFixed(assetMeta(s.selected).decimals + 1)));
    setTarget(Number((px + dir * 2.5 * a).toFixed(assetMeta(s.selected).decimals + 1)));
    setTrigger(null);
    setOverride(false);
  ...
  const begin = () => {
    // first-ever order: legal gate first, then the emotional check-in
    onTradeIntent();
    if (s.tradeDisclaimerShown) setCheckinOpen(true);
  };
  const submit = (checkin: Checkin) => {
    dispatch({ type: "PLACE_ORDER", symbol: s.selected, orderType, side, qty, trigger, stop, target, setup, checkin, override });
    ...
    <div className="panel p-4 flex flex-col gap-3.5 sticky top-0">
      ...
      {cooldownLeft > 0 && (
        <div className="panel-inset p-3 text-center" style={{ borderColor: "rgba(224,163,59,0.45)" }}>
          ...
          <p className="text-[11.5px] text-fog-400 mt-1">{s.tiltReason ?? "Emotional trading pattern detected."}</p>
        ...
        <div className="grid grid-cols-3 gap-1 p-1 rounded-lg" style={{ background: "#0a1120", border: "1px solid #1c2942" }}>
          {(["market", "limit", "stop"] as OrderType[]).map((t) => (
            <button key={t} onClick={() => setOrderType(t)}
              ...
              style={orderType === t ? { background: "#24344f", color: "#eef3fa" } : { color: "#6b7d96" }}>{t}</button>
          ))}
        </div>
      </div>
      ...
      <div className="panel-inset p-3 space-y-1.5 num text-[12px]">
        ...
        <Row k="Planned risk" v={`$${risk$.toFixed(0)} · ${s.equity > 0 ? ((risk$ / s.equity) * 100).toFixed(2) : "0"}%`} tone={overPlan ? "#e0564f" : "#2fb98c"} />
