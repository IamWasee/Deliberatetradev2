      ...
      <div className="max-w-[980px] mx-auto space-y-3.5">
        ...
        <div className="panel p-4">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <input className="field !w-[120px] !py-1 !text-[11.5px] num uppercase" placeholder="SYMBOL…" maxLength={6}
  ...
  return (
    <div className="rounded-lg overflow-hidden transition-all" style={{ background: "#0a1120", border: `1px solid ${open ? "#2a3c5e" : "#16213a"}` }}>
      <button onClick={onToggle} className="w-full flex items-center gap-3 px-3.5 py-2.5 text-left row-hover">
        ...
        {t.violations.length > 0 && (
          <span className="lbl !text-[8.5px] px-1.5 py-0.5 rounded text-down" style={{ border: "1px solid rgba(224,86,79,0.4)" }}>{t.violations.length} VIOL.</span>
        )}
        {t.journal ? (
          ...
          <span className="ml-auto lbl !text-[9px] text-amber" style={{ border: "1px solid rgba(224,163,59,0.4)", padding: "2px 6px", borderRadius: 6 }}>JOURNAL DUE</span>
        )}
        <span className={`num font-semibold text-[13px] w-[84px] text-right ${t.pnl >= 0 ? "text-up" : "text-down"}`}>{fmtSigned(t.pnl, 0)} · {fmtR(t.r)}</span>
      </button>
      {open && t.journal && (
        <div className="px-4 pb-4 pt-1 border-t border-line-soft animate-fade-in space-y-3">
          <div className="grid md:grid-cols-2 gap-3 text-[12px] leading-relaxed">
            ...
            <div><p className="lbl mb-1">Emotions</p><p className="text-fog-300">during: <strong>{emotionLabel(t.journal.emotionDuring)}</strong> · after: <strong>{emotionLabel(t.journal.emotionAfter)}</strong></p>
              <p className="text-fog-400 mt-1">Rules followed: <strong style={{ color: t.journal.followedRules === "yes" ? "#2fb98c" : "#e0564f" }}>{t.journal.followedRules.toUpperCase()}</strong>{t.journal.rulesNote && ` — ${t.journal.rulesNote}`}</p></div>
          ...
          <div className="panel-inset p-3.5">
            ...
            <p className="text-[12.5px] text-fog-200 leading-relaxed">{t.journal.debrief}</p>
          </div>
        </div>
      )}
    </div>
