/* =====================================================================
   ...
   rules so the standalone build stays fully functional.
   ...
   ===================================================================== */
import { emotionLabel, type EmotionTag, type Plan, type Trade, type Violation, type Mission } from "./types";
...
export type TiltType =
  | "size-up-after-loss" | "rapid-reentry" | "setup-abandon"
  | "rule-break-after-loss" | "overtrading-burst" | "revenge-flip";

export interface TiltSignal { key: string; type: TiltType; severity: 1 | 2 | 3; at: number; detail: string }
...
export function detectTiltSignals(trades: Trade[], violations: Violation[]): TiltSignal[] {
  ...
  for (const t of byExit) {
    ...
    const next = byExit.slice(i + 1, i + 4);

    for (const n of next) {
      if (t.riskAmount > 0 && n.riskAmount >= t.riskAmount * 1.5) {
        const pct = Math.round((n.riskAmount / t.riskAmount - 1) * 100);
        out.push({
          key: "size:" + t.id + ":" + n.id, type: "size-up-after-loss", severity: 2, at: n.entryTs,
          detail: "Risk rose $" + t.riskAmount.toFixed(0) + " to $" + n.riskAmount.toFixed(0) +
            " (+" + pct + "%) within 3 trades of a loss.",
        });
        break;
      }
    }
    ...
    if (imm && imm.side !== t.side && imm.entryTs - t.exitTs <= RAPID_MS &&
      ...
      out.push({
        ...
        detail: "Flipped " + t.side + " to " + imm.side + " with " +
          Math.round((imm.riskAmount / t.riskAmount) * 100) + "% of the losing trade's risk, " +
          ((imm.entryTs - t.exitTs) / 60000).toFixed(1) + " min after the loss.",
      });
    }
    ...
    if (streak >= 2 && usual.size > 0) {
      ...
      if (after && !usual.has(after.setup)) {
        out.push({
          ...
          detail: "After " + streak + " straight losses, left the usual setups (" +
            [...usual].join(", ") + ') for "' + after.setup + '".',
        });
      }
    }
    ...
    if (near.length > 0) {
      const minsAfter = (Math.round((near[0].ts - t.exitTs) / 600)) / 100;
      out.push({
        key: "break:" + t.id + ":" + near[0].id, type: "rule-break-after-loss", severity: 3, at: near[0].ts,
        detail: near[0].rule + " - " + minsAfter.toFixed(1) + " min after a losing close.",
      });
    } else {
