...
/* =====================================================================
   3 · READINESS SCORE — gates first, behavior drives the number.
   ===================================================================== */
export const READINESS_GATES = {
  minTrades: 50, minDays: 15, cleanWindowDays: 14, minRecentProcess: 65,
};
...
export interface ReadinessGate { id: string; label: string; pass: boolean; detail: string }
export interface ReadinessResult {
  score: number; stage: ReadinessStage; stageIdx: number;
  gates: ReadinessGate[];
  components: { key: string; label: string; value: number }[];
  feedback: string[];
}

function activeDays(trades: Trade[]): number {
  return new Set(trades.map((t) => new Date(t.exitTs).toDateString())).size;
}

export function computeReadiness(trades: Trade[], violations: Violation[], plan: Plan | null): ReadinessResult {
  const signals = detectTiltSignals(trades, violations);
  const days = activeDays(trades);
  const recent30 = trades.slice(-30);
  const recentProcess = recent30.length >= 10 ? computeProcess(recent30, violations, plan, signals).score : 0;

  const seriousRecent = violations.filter(
    (v) => v.ts >= Date.now() - READINESS_GATES.cleanWindowDays * DAY && SERIOUS_RE.test(v.rule));

  const gates: ReadinessGate[] = [
    {
      id: "trades", label: `Minimum ${READINESS_GATES.minTrades} closed trades`,
      pass: trades.length >= READINESS_GATES.minTrades,
      detail: `${trades.length}/${READINESS_GATES.minTrades} closed`,
    },
    {
      id: "days", label: `Minimum ${READINESS_GATES.minDays} active trading days`,
      pass: days >= READINESS_GATES.minDays,
      detail: `${days}/${READINESS_GATES.minDays} days`,
    },
    {
      id: "clean", label: `No serious violations in the last ${READINESS_GATES.cleanWindowDays} days`,
      pass: seriousRecent.length === 0,
      detail: seriousRecent.length === 0 ? "clean window" : `${seriousRecent.length} serious violation${seriousRecent.length > 1 ? "s" : ""}`,
    },
    {
      id: "process", label: `Recent process average ≥ ${READINESS_GATES.minRecentProcess}`,
      pass: recent30.length >= 10 && recentProcess >= READINESS_GATES.minRecentProcess,
      detail: recent30.length >= 10 ? `recent process ${recentProcess}` : `needs ≥10 recent trades`,
    },
  ];

  /* components — quality of behavior, not volume */
  const last75 = trades.slice(-75);
  const c = {
    recentProcess: recent30.length >= 10 ? recentProcess / 100 : 0,
    adherence: adherenceComponent(last75, violations, 30),
    postLoss: postLossComponent(trades, signals),
    consistency: (() => {
      if (days === 0) return 0;
      const byDay = new Map<string, { pnl: number; bad: boolean }>();
      for (const t of trades) {
        const k = new Date(t.exitTs).toDateString();
        const d = byDay.get(k) ?? { pnl: 0, bad: false };
        d.pnl += t.pnl;
        if (t.violations.length > 0 || t.override) d.bad = true;
        byDay.set(k, d);
      }
      const limit = (plan?.maxDailyLossPct ?? 3) / 100 * (plan?.startingCapital ?? 25000);
      let good = 0;
    ...
    setupEmotion: (setupComponent(trades, plan) + emotionComponent(trades)) / 2,
  };

  const W = { recentProcess: 25, adherence: 25, postLoss: 20, consistency: 15, sample: 10, setupEmotion: 5 };
  let raw = 0;
  (Object.keys(W) as (keyof typeof W)[]).forEach((k) => { raw += W[k] * clamp01(c[k]); });
  ...
  /* feedback — why this stage, what to fix next */
  const feedback: string[] = [];
  gates.filter((g) => !g.pass).forEach((g) => feedback.push(`Gate: ${g.label.toLowerCase()} — ${g.detail}.`));
  const gaps: [string, number, string][] = [
    ["Post-loss behavior", c.postLoss, "After losses, keep size flat and wait. Revenge trades are the single biggest readiness killer."],
    ["Risk rule adherence", c.adherence, "Let the sizing calculator run every order — zero violations for two straight weeks moves this fast."],
    ["Recent process", c.recentProcess, "Your most recent 30 trades set the tone. Slow down and trade only A-grade setups."],
    ["Day-to-day consistency", c.consistency, "Aim for clean days: no violations, and daily losses inside half your limit."],
    ["Setup & emotional discipline", c.setupEmotion, "Stick to your two best setups and journal the emotional state honestly."],
    ["Sample depth", c.sample, "Reach ~100 trades across ~30 days so the sample actually proves something."],
  ];
  ...
  if (feedback.length === 0) feedback.push("Hold the standard. Readiness is rented daily, not owned.");

  return {
    score, stage, stageIdx, gates,
    components: [
