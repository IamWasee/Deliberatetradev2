/* =====================================================================
   ...
   low-effort ones score 10–30 and drag the (low-weight) journal
...
const REPEAT_RE = /(.)\1{5,}/;
const WORD_REPEAT_RE = /\b(\w{2,})\b(?:\s+\1\b){4,}/i;
const KEYBOARD_MASH = /^[^aeiou\s]{6,}$/i;
...
function fieldIsGarbage(t: string): boolean {
  ...
  if (words.length === 0) return s.replace(/[^a-z0-9]/gi, "").length > 8;
