/* Legal surfaces: signup consent text, persistent footer, first-trade
   simulation warning, and the reusable consent checkbox. */
...
/** Consent checkbox used on the plan-signature step. */
  ...
  return (
    <label className="flex items-start gap-2.5 p-3 rounded-lg cursor-pointer transition-all duration-150"
      ...
      <input type="checkbox" checked={checked} onChange={(e) => onChange(e.target.checked)}
        className="mt-0.5 accent-[#39c5a5]" />
...
/** Persistent disclaimer footer on every page. */
export function DisclaimerFooter({ onLegal }: { onLegal?: () => void }) {
  return (
    <div className="shrink-0 border-t border-line px-4 py-2 flex flex-wrap items-center gap-x-3 gap-y-1"
      style={{ background: "rgba(7,12,22,0.72)" }}>
...
/** Shown once — the first time the user attempts to place a trade. */
...
/** Mount once inside the app shell: pops on the first order attempt. */
